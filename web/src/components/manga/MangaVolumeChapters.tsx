...
import { Manga, MangaSingle, MangaVolume, MangaChapter } from '../../types/manga'
import { humanFileSize } from '../../tools/general'
import { QueueContext } from "../../contexts/QueueContext"
import { Download } from '../../icons'

export default function MangaVolumeChapters({
    manga,
    volume
}:{
    manga: MangaSingle,
...
})
{
    ...
    const download_name = `${manga?.name} - ${volume.name}.${volume.ext}`
    ...
    return (
        ...
        >
            <div className="grid py-2 px-2 sm:px-4 bg-zinc-600/60 shadow-md single-file-row">
                ...
                <div style={{'gridArea':'eng_name'}} className="text-xs text-gray-400">{ volume.status ? volume.status : '' }</div>
                { volume.filesize > 0 && (
                <>
            ...
            <div className="flex flex-col divide-y divide-zinc-700/20">
                { volume?.chapters && volume.chapters.map( (chapter) => {
                    return ( <MangaVolumeChapter key={`v${volume.id}c${chapter.id}`} chapter={chapter} volume={volume} manga={manga} /> )
                } ) }
            </div>
        </div>
    )
...
function MangaVolumeChapter({chapter, volume, manga}:{chapter: MangaChapter, volume:MangaVolume,  manga:Manga})
{
    const queueContext = useContext(QueueContext)

    const download_link = chapter.filesize > 0 ? chapter.download_path : undefined
    const download_name = `${manga?.name} - ${volume.name} - ${chapter.name}.${chapter.ext}`
