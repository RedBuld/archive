...
import { MangaSingle, MangaVolume } from '../../types/manga'
...
})
{
    return (
        <div className="grid w-full max-w-screen-2xl grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
            { manga.volumes?.length ? manga.volumes.map( (volume) => { return (
                <MangaVolumeCard key={volume.id} volume={volume} manga={manga} />
            ) } ) : (
...
})
{
    ...
    const download_name = volume.filesize > 0 ? `${manga?.name} - ${volume.name}.${volume.ext}` : undefined
